(function() {
  ...
  Level.prototype.putFloor = function(start, end) {
    for (var i = start; i < end; i++) {
      if (!this.statics[13]) this.statics[13] = [];
      if (!this.statics[14]) this.statics[14] = [];
  ...
  Level.prototype.putWall = function(x, y, height) {
    ...
    for (var i = y-height; i < y; i++) {
      if (i < 0) continue;
      if (!this.statics[i]) this.statics[i] = [];
  ...
  Level.prototype.putPipe = function(x, y, height) {
    for (var i = y - height; i < y; i++) {
      if (i < 0) continue;
      if (!this.statics[i]) this.statics[i] = [];
      if (!this.statics[i][x]) this.statics[i][x] = null;
      if (!this.statics[i][x+1]) this.statics[i][x+1] = null;
      if (i === y - height) {
  ...
  Level.prototype.putLeftPipe = function(x,y) {
    if (!this.statics[y]) this.statics[y] = [];
    if (!this.statics[y+1]) this.statics[y+1] = [];
  ...
  Level.prototype.putCloud = function(x, y) {
    if (!this.scenery[y]) this.scenery[y] = [];
  ...
  Level.prototype.putBigHill = function(x, y) {
    var px = x*16, py = y*16;
    if (!this.scenery[y]) this.scenery[y] = [];
    if (!this.scenery[y-1]) this.scenery[y-1] = [];
    if (!this.scenery[y-2]) this.scenery[y-2] = [];
  ...
  Level.prototype.putFlagpole = function(x) {
    if (!this.statics[12]) this.statics[12] = [];
    for (var i=3; i < 12; i++) {
      if (!this.scenery[i]) this.scenery[i] = [];
      this.scenery[i][x] = new Mario.Prop([16*x, 16*i], this.flagpoleSprites[1]);
    }
    if (!this.scenery[2]) this.scenery[2] = [];
